...
export async function getCustomerStatus() {
    return await client.db('b39wd').collection('hallbook').aggregate([
        { $lookup: { from: "roomcreate", localField: "fk", foreignField: "roomId", as: "RoomDetails" } },
...
export async function getRoomDetails(date,startTime,endTime) {
    return await client.db('b39wd').collection('hallbook').findOne({date:date,startTime:startTime,endTime:endTime});
}
